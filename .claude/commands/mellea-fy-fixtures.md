...
>
> **READ the schema FIRST.** Before drafting the emission JSON, open `.claude/schemas/fixtures_emission.schema.json` via the `Read` tool and read it in full. The schema is authoritative — every field name in your emission must appear in the schema's `properties`; every name in the schema's `required` list MUST be present; `additionalProperties: false` means literally no extra fields. Empirically observed rejections 2026-05-20: emissions with `fixture_id` / `name` / `expected_output_checks` / `format_version` were rejected because the canonical schema uses `id` and a closed set of per-fixture properties. Do NOT infer field names from training memory; the schema decides. After drafting, re-open the schema and walk every field of your emission against `fixtures_emission.schema.json:properties` to self-check.
